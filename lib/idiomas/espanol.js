...
const smsAvisoAG = () => { return `*ADVERTENCIA ⚠️*\n\n` }
const smsAvisoIIG = () => { return `*INFORMACIÓN ❕*\n\n` }
const smsAvisoFG = () => { return `*ERROR ❌*\n\n` }
const smsAvisoMG = () => { return `*ACCIÓN MAL USADA ❗️*\n\n` }
...
const smsMods = () => { return `\`\`\`THIS COMMAND ONLY MODERATORS AND MY CREATOR CAN USE I T!!\`\`\`\`` }
const smsPremium = () => { return `\`\`\`THIS COMMAND IS ONLY AVAILABLE FOR PREMIUM USERS AND MY CREATOR(A)!! TO GET PREMIUM BUY A PASS USING EX : /owner\`\`\`` }
...
const smsUnreg = () => { return `\`\`\`YOU NEED TO BE REGISTERED TO USE THIS COMMAND, WRITE EX : /make-account name.age TO REGISTER !!\`\`\`` }
...
//config.js
const smsMeWait = () => { return '_*Please wait...*_'} 
