...
const handler = async (m, {conn, args, usedPrefix, command}) => {
  if (!args[0]) throw '*Incorrect use of the command, enter a YouTube link.*';
  let enviando;
  if (enviando) return  
      enviando = true    
  let youtubeLink = '';
  ...
  const { key } = await m.reply(`*_Please wait..._*`);
  try {
   ...
   if (fileSizeInMB > 100) {
    await conn.sendMessage(m.chat, {document: buff, caption: `*▢ Title :* ${ttl_1}\n*▢ Video Weight :* ${roundedFileSizeInMB} MB`, fileName: ttl_1 + '.mp4', mimetype: 'video/mp4'}, {quoted: m});
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        await conn.sendMessage(m.chat, {text: `*The video cannot be downloaded or sent, try again.*`, edit: key}, {quoted: m});
