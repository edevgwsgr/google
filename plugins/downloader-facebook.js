...
if (!args[0]) return conn.reply(m.chat, `Ex : ${usedPrefix + command} url`, fkontak, m)
if (!args[0].match(/www.facebook.com|fb.watch/g)) return conn.reply(m.chat, `Ex : ${usedPrefix + command} facebook.com/watch?v=636541475139`, fkontak, m)
try {
...
conn.sendFile(m.chat, r2es, 'error.mp4', `*_Charging..._ ▬▬▬▬▬▬▬*`, m);
} catch (err1) {
try {
...
conn.sendFile(m.chat, VIDEO, 'error.mp4', `*_Charging..._ ▬▬▬▬▬▬▬*`, m);
} catch (err2) {
try {
const ress = await fg.fbdl(args[0]);
const urll = await ress.data[0].url;
await conn.sendFile(m.chat, urll, 'error.mp4', `*_Charging..._ ▬▬▬▬▬▬▬*`, m);
} catch (err3) {
try {
const res = await fbDownloader(args[0]);
for (const result of res.download) {
const ur = result.url;
await conn.sendFile(m.chat, ur, 'error.mp4', `*_Charging..._ ▬▬▬▬▬▬▬*`, m);
}
} catch (err4) {
try {
...
await conn.sendFile(m.chat, url3, 'error.mp4', `*_Charging..._ ▬▬▬▬▬▬▬*`, m);
} catch (err5) {
try {
const {result} = await facebookdl(args[0]).catch(async (_) => await facebookdlv2(args[0])).catch(async (_) => await savefrom(args[0]));
for (const {url, isVideo} of result.reverse()) await conn.sendFile(m.chat, url, `facebook.${!isVideo ? 'bin' : 'mp4'}`, `𝙑𝙄𝘿𝙀𝙊 𝘿𝙀 𝙁𝘼𝘾𝙀𝘽𝙊𝙊𝙆`, m);
} catch (err6) {
